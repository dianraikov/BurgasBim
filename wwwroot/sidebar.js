...
async function getVersions(hubId, projectId, itemId) {
    ...
    return versions.map(version => createTreeNode(`version|${version.id}`, `Please click only once: ${version.attributes.createTime}`, 'icon-version')); // Prevenir la acción predeterminada del doble clic
}
